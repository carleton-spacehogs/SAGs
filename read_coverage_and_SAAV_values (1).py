...
if __name__ == '__main__':
	...
	SAG_colors = {'C09':'r','E23':'g','K20':'y','M21':'b','N22':'k'}
	shapes = ['*','d','>','p','s','o']
	...
	for key in color_label_dict.keys():
		...
		shapes_A = []
		c = 0
		while len(shapes_A) != len(colors_A):
			shapes_A.append(shapes[c % len(shapes)])
		...
		for sag in SAGs:
			...
			for line in lines[1:]:
				...
				if line[0] == sag:
					if line[4] == '0.0':
						...
						y.append(math.log10(1.0E-10))
					else:
						y.append(math.log10(float(line[4])))
					...
					if key == 'A':
						...
						specific_shapes.append(shapes_A[index])
					if key == 'B':
			...
			fart = seaborn.JointGrid('x','y',data=pd,)
			fart.ax_joint.cla()
			plt.sca(fart.ax_joint)
			for i in range(len(specific_colors)):
				plt.scatter(x[i], y[i], c=specific_colors[i],marker=specific_shapes[i],edgecolor='none')
			#plt.xlabel('Average SAAV/SNV ratio * no. of metagenomic reads')
			#plt.ylabel('log( Average metatranscriptome coverage )')
			#plt.suptitle(sag)
			fart.plot_marginals(seaborn.distplot, kde=True, color = 'k')
			plt.show()
			
			#plt.savefig(sag + '_double_scatter_reg_newest_' + key + '.png')
		#print(color_label_dict[key][i])
		print(key,'\n')
		for i in range(len(colors_A)):
			print(mp.colors.rgb2hex(colors_A[i]),shapes_A[i],' : ',color_label_dict[key][i])
			#color += 1
